...
def fetch_all_games():
    ...
    while True:
        print(f"Fetching games with offset: {offset}")
        response = supabase.table('steamsearcher_duplicate').select(
            'appid, name, genre, recommendation_count, description_phrases, keyphrase, summary'
...
def embed_and_save_game_data(model_path, embed_path):
    ...
    for game in games:
        ...
        embeddings.append({
            ...
            'keyphrase': game['keyphrase'],
            'summary': game['summary'],
            'embedding': game_embedding.tolist()  # Convert numpy array to list for pickle compatibility
        })
