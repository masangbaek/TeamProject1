...
def search_games(query, model, embeddings_df):
    ...
    for _, row in embeddings_df.iterrows():
        ...
        game_embedding = row['embedding']
        query_vector = np.mean([model.wv[word] for word in query_words if word in model.wv], axis=0)
        if len(query_vector) == 0 or len(game_embedding) == 0:
            continue
        similarity = cosine_similarity([query_vector], [game_embedding])[0][0] * 100
        results.append({
            'appid': row['appid'],
            'name': row['name'],
            'genre': row['genre'],
        ...
        seen_game_titles.add(game_title)  # 추가: 중복 방지
...
# GloVe 기반 유사도 계산 함수
def find_most_similar_rows(query, embeddings_df, top_n=5):
...
def steam_searcher_list(request):
    ...
    # 중복 게임명 제거 (딕셔너리일 경우)
    seen_game_names = set()
    unique_glove_results = []
    for game in glove_results:
        # game이 dict 형태일 경우
        if isinstance(game, dict):
            game_name = game.get('name')
            if game_name and game_name not in seen_game_names:
                seen_game_names.add(game_name)
                unique_glove_results.append(game)
        else:
            # game이 dict 형태가 아닐 경우 추가 처리 필요
            # 예: game 객체가 직접적으로 중복을 관리할 수 있는 경우
            pass
        print("GloVe 검색 결과의 예시:")
        for item in glove_results[:5]:  # 처음 5개 항목을 출력
            print(item)
...
def get_reviews_by_category(category):
    try:
        response = supabase.table('reviews_with_labels').select('*').eq('labels', category).limit(10).execute()
