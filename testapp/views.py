...
#     unique_games = []
#     for game in games:
#         if game['name'].lower() not in seen_names:
#             unique_games.append(game)
#             seen_names.add(game['name'].lower())
#     return unique_games
#
# # 텍스트 벡터화 함수
...
#         if word in model.wv:
#             similar_words.update([w for w, _ in model.wv.most_similar(word, topn=10)])
#         else:
...
#         similarity = cosine_similarity([query_vector], [game_embedding])[0][0]
#
#         results.append({
#             'appid': row['appid'],
#             'name': row['name'],
...
def steam_searcher_list(request):
    ...
    games = SteamSearcher.objects.filter(
        Q(name__icontains=search_query) |
        Q(genre__icontains=search_query)
    ).order_by('-recommendation_count') # 추천수를 기준으로 내림차순 정렬
    # ).order_by('name')  이름으로 정렬
